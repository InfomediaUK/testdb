...
<bean:message key="title.locationOrder"/>

<br/>
<br/>
<bean:write name="SiteDisplayOrderFormAdmin" property="client.name"/>
<br/>
<br/>
<bean:write name="SiteDisplayOrderFormAdmin" property="site.name"/>
<br/>
<br/>

<logic:present name="SiteDisplayOrderFormAdmin" property="list">
<form action="locationOrderProcess.do" name="SiteDisplayOrderFormAdmin">
  <html:select name="SiteDisplayOrderFormAdmin" property="commaDelimitedKey" size="20" style="width: 400px">
  <logic:iterate id="location" name="SiteDisplayOrderFormAdmin" property="list" type="com.helmet.bean.Location">
    <bean:define id="id" name="location" property="locationId" />
    <bean:define id="noOfChanges" name="location" property="noOfChanges" />
    <bean:define id="name" name="location" property="name" />
    <option value="<%= id %>,<%= noOfChanges %>"><%= name %></option>
  </logic:iterate>
  </html:select>
  <br/>
  <br/>
  <html:button onclick="javascript:swap(document.SiteDisplayOrderFormAdmin.commaDelimitedKey, -1);" property="up">
    <bean:message key="button.up"/>
  </html:button>
  <html:button onclick="javascript:swap(document.SiteDisplayOrderFormAdmin.commaDelimitedKey, 1);" property="down">
    <bean:message key="button.down"/>
  </html:button>
  <html:submit onclick="javascript:loadvalues(document.SiteDisplayOrderFormAdmin.commaDelimitedKey, document.SiteDisplayOrderFormAdmin.order)">
...
--%>
  <html:submit onclick="javascript:zeroiseDisplayOrder.value='true';loadvalues(document.SiteDisplayOrderFormAdmin.commaDelimitedKey, document.SiteDisplayOrderFormAdmin.order)">
    <bean:message key="button.zeroise"/>
  </html:submit>
  <html:hidden name="SiteDisplayOrderFormAdmin" property="order"/>
  <html:hidden name="SiteDisplayOrderFormAdmin" property="zeroiseDisplayOrder"/>
  <html:hidden name="SiteDisplayOrderFormAdmin" property="site.siteId"/>
</form>
</logic:present>
...
function showIt() {
  alert(document.SiteDisplayOrderFormAdmin.order.value);
}
