...
<bean:message key="title.bookingSearch"/>
		</td>
  </tr>
</table>
<html:errors/>
<table width="100%" cellpadding="0" cellspacing="0" border="0">
<html:form action="/bookingSearchProcess.do" focus="bookingId" onreset="resetDates()" onsubmit="return singleSubmit();">
  <tr>
    <td width="80%">
<table class="simple" width="100%">
  ...
  <tr>
    <th>
      <bean:message key="label.bookingStatus"/>
    </th>
    <td>
      <html:select property="bookingStatus" tabindex="1">
<html:option value=""><bean:message key="label.pleaseSelect"/></html:option>
<% 
for (int i = 0; i < com.helmet.bean.Booking.BOOKING_STATUS_DESCRIPTION_KEYS.length; i++) {
%>
<html:option value="<%= String.valueOf(i) %>"><bean:message key="<%= com.helmet.bean.Booking.BOOKING_STATUS_DESCRIPTION_KEYS[i] %>"/></html:option>
<%
}
%>
      </html:select>
...
<script type="text/javascript">
  ...
  function clearForm() {
    document.BookingSearchFormMgr.bookingId.value = '';
    document.BookingSearchFormMgr.bookingStatus.value = '';
    document.BookingSearchFormMgr.singleCandidate.value = '';
    document.BookingSearchFormMgr.siteId.value = '';
    document.BookingSearchFormMgr.locationId.value = '';
    document.BookingSearchFormMgr.jobProfileId.value = '';
    document.BookingSearchFormMgr.fromDate.value = '';
    document.BookingSearchFormMgr.toDate.value = '';
    loadSites();
    resetDates();
  }
...
//  for (i = 0; i < document.BookingSearchFormMgr.elements.length; i++) {
//    alert(document.forms['BookingSearchFormMgr'].elements[i].name + " " + document.forms['BookingSearchFormMgr'].elements[i]);
//  }
//  return document.forms['BookingSearchFormMgr'].elements[selectField];

//  alert(selectField);
//  var x = document.getElementById(selectField);
//  alert(x.name);
//  return x;
...
/*
  var x;
  for (i = 0; i < document.BookingSearchFormMgr.elements.length; i++) {
    if (document.BookingSearchFormMgr.elements[i]) {
	    if (document.BookingSearchFormMgr.elements[i].name) {
alert(selectField + " - " + document.BookingSearchFormMgr.elements[i].name);
	      if(document.BookingSearchFormMgr.elements[i].name.toLowerCase() == selectField.toLowerCase()) {
	        x = document.BookingSearchFormMgr.elements[i];
	        break;
	      }
	    }
	  }
...
function doYesterday() {
  ...
  document.BookingSearchFormMgr.fromDate.value = yesterdayStr;
  document.BookingSearchFormMgr.toDate.value = yesterdayStr;
}

function doToday() {
  var todayStr = getDateStr(getDate(0));
  document.BookingSearchFormMgr.fromDate.value = todayStr;
  document.BookingSearchFormMgr.toDate.value = todayStr;
}

function doTomorrow() {
  var tomorrowStr = getDateStr(getDate(+1));
  document.BookingSearchFormMgr.fromDate.value = tomorrowStr;
  document.BookingSearchFormMgr.toDate.value = tomorrowStr;
}
...
function doLastWeek() {
  document.BookingSearchFormMgr.fromDate.value = getDateStr(getSOW(getDate(-7)));
  document.BookingSearchFormMgr.toDate.value = getDateStr(getEOW(getDate(-7)));
}

function doThisWeek() {
  document.BookingSearchFormMgr.fromDate.value = getDateStr(getSOW(getDate(0)));
  document.BookingSearchFormMgr.toDate.value = getDateStr(getEOW(getDate(0)));
}
